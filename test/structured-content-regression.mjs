...
console.log(`${BLUE}🧪 cBioPortal Structured Content Regression Tests${RESET}`);

const toolExpectations = [
  {
    path: 'src/tools/mutation-frequency.ts',
    required: ['createCodeModeResponse', 'createCodeModeError'],
  },
  {
    path: 'src/tools/study-summary.ts',
    required: ['createCodeModeResponse', 'createCodeModeError'],
  },
];
...
assertContains('src/index.ts', indexContent, 'CbioportalDataDO', 'index.ts exports CbioportalDataDO');
...
console.log(`\n${GREEN}✅ cBioPortal structured content regression tests passed.${RESET}`);
